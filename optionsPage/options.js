...
function checkAllCD() {
  ...
  chrome.storage.sync.set({"sepAllCD": numChecked === cdCodes.length});
}
...
function checkAllDVD() {
  ...
  chrome.storage.sync.set({"sepAllDVD": numChecked === dvdCodes.length});
}
...
function checkOther() {
  ...
  chrome.storage.sync.set({"sepOther": numChecked === otherCodes.length});
}
...
function toggleAllCD() {
  for (var i = 0; i < cdCodes.length; i++) {
    ...
    chrome.storage.sync.set({ [cdCodes[i]]: document.getElementById(cdCodes[i]).checked });
  }
}
...
function toggleAllDVD() {
  for (var i = 0; i < dvdCodes.length; i++) {
    ...
    chrome.storage.sync.set({ [dvdCodes[i]]: document.getElementById(dvdCodes[i]).checked });
  }
}
...
function toggleOther() {
  for (var i = 0; i < otherCodes.length; i++) {
    ...
    chrome.storage.sync.set({ [otherCodes[i]]: document.getElementById(otherCodes[i]).checked });
  }
}
...
skin.addEventListener('change', function() {
  chrome.storage.sync.set({"skin": skin.value}, function(res) {
    chrome.runtime.sendMessage({"key": "updateExtensionIcon"});
  });
});
document.getElementById("patronMsgSwitch").addEventListener('click', function() {
  chrome.storage.sync.set({"patronMsg": patronMsg.checked});
});
