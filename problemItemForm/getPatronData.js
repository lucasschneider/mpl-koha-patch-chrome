(function(){
  ...
  if (patronInfo && patronInfo.length > 0) {
    data.patronName = patronInfo[0].children[0].textContent.replace(/\s\s+/g, ' ').replace(/\./g,'').slice(0, -17).replace(/\w+/g, function(w){return w[0].toUpperCase() + w.slice(1).toLowerCase()});
